...
import { Users, ShieldAlert, Flag, TrendingUp, Search, Check, X, Loader2, Crown, User } from "lucide-react";

const ADMIN_EMAILS = ["admin@jodibanao.com"]; // <-- Add your admin email here
...
export default function AdminPage() {
  ...
  const [stats, setStats] = useState({ totalUsers: 0, reports: 0, recentSignups: 0 });
    ...
    <div className="min-h-screen bg-muted/20 flex flex-col">
      ...
      <div className="container mx-auto px-4 max-w-7xl py-8 flex flex-col gap-8">
        ...
        <Tabs defaultValue="users" className="w-full">
          <TabsList className="bg-white border border-border shadow-sm rounded-xl h-12 p-1 gap-1">
                    ...
                    <tbody className="divide-y divide-border">
                      {filteredUsers.slice(0, 50).map(u => (
                        <tr key={u.id} className="hover:bg-muted/20 transition-colors">
                          <td className="px-4 py-3">
                            <div className="flex items-center gap-3">
                              ...
                              <div>
                                ...
                                <p className="text-xs text-muted-foreground">{u.gender || "—"} • {u.religion || "—"}</p>
                              </div>
                            </div>
                          </td>
                          ...
                          <td className="px-4 py-3">
                            <Badge className={
                              u.memberships?.[0]?.plan === "gold" ? "bg-amber-100 text-amber-700 border-amber-200" :
                              u.memberships?.[0]?.plan === "silver" ? "bg-blue-100 text-blue-700 border-blue-200" :
                              u.memberships?.[0]?.plan === "platinum" ? "bg-purple-100 text-purple-700 border-purple-200" :
                              "bg-muted text-muted-foreground border-border"
                            }>
                              {u.memberships?.[0]?.plan || "free"}
                            </Badge>
                          </td>
